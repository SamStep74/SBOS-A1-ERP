...
const { buildEInvoiceXml, eInvoiceTotals, EINVOICE_NAMESPACE } = require("./einvoice.cjs");
...
test("einvoice: supplier carries the VAT-payer reg number (ԱԱՀՎՀՀ)", () => {
  const xml = buildEInvoiceXml({ ...sample, supplier: { name: "Ս", hvhh: "00123456", vatId: "1234567" } });
...
test("einvoice: a line carries unit price, excise and env-fee; total includes them", () => {
  const xml = buildEInvoiceXml({
    ...sample,
    lines: [{ description: "Ապ", quantity: 2, netAmount: 100000, vatRate: 20, exciseAmount: 5000, envFee: 1000 }],
  });
