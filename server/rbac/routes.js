...
function registerRbacRoutes(app, opts = {}) {
  ...
  app.get('/api/rbac/permissions', { preHandler: requirePermFastify('security.permission_set.read') }, async (request) => {
    return {
  ...
  app.get('/api/rbac/permissions/:key', { preHandler: requirePermFastify('security.permission_set.read') }, async (request, reply) => {
  ...
  app.get('/api/rbac/permission-sets', { preHandler: requirePermFastify('security.permission_set.read') }, async () => {
    return {
  ...
  app.get('/api/rbac/permission-sets/:id', { preHandler: requirePermFastify('security.permission_set.read') }, async (request, reply) => {
  ...
  app.post('/api/rbac/roles', { preHandler: requirePermFastify('security.role.create') }, async (request, reply) => {
  ...
  app.patch('/api/rbac/roles/:id', { preHandler: requirePermFastify('security.role.update') }, async (request, reply) => {
  ...
  app.delete('/api/rbac/roles/:id', { preHandler: requirePermFastify('security.role.delete') }, async (request, reply) => {
    const id = request.params.id;
    const r = getRole(id);
    if (!r) return reply.code(404).send({ error: 'not_found' });
    if (r.isSystem) return reply.code(409).send({ error: 'system_role_immutable' });
    // Refuse if anyone still holds this role.
    const used = db.prepare(`SELECT COUNT(*) AS c FROM sbos_rbac_user_roles WHERE role_id = ?`).get(id);
  ...
  app.get('/api/rbac/users/:userId/effective', { preHandler: requirePermFastify('security.user.read') }, async (request, reply) => {
  ...
  app.post('/api/rbac/users/:userId/permission-sets', { preHandler: requirePermFastify('security.role.assign') }, async (request, reply) => {
    const userId = Number(request.params.userId);
    const body = request.body || {};
  ...
  app.delete('/api/rbac/users/:userId/permission-sets/:ps', { preHandler: requirePermFastify('security.role.assign') }, async (request, reply) => {
    const userId = Number(request.params.userId);
    const psId = String(request.params.ps);
    const tenant = db.prepare(`SELECT tenant_id FROM users WHERE id = ?`).get(userId);
    if (!tenant) return reply.code(404).send({ error: 'user_not_found' });
    db.prepare(`DELETE FROM sbos_rbac_user_permission_sets WHERE user_id = ? AND permission_set_id = ? AND tenant_id = ?`)
      .run(userId, psId, tenant.tenant_id || 0);
    return reply.code(204).send();
  });

  app.post('/api/rbac/users/:userId/role', { preHandler: requirePermFastify('security.role.assign') }, async (request, reply) => {
    const userId = Number(request.params.userId);
    const body = request.body || {};
  ...
  app.get('/api/rbac/field-policies', { preHandler: requirePermFastify('security.permission_set.read') }, async () => {
    return {
  ...
  app.put('/api/rbac/field-policies/:path(*)', { preHandler: requirePermFastify('security.permission_set.update') }, async (request, reply) => {
  ...
  app.get('/api/rbac/record-rules', { preHandler: requirePermFastify('security.permission_set.read') }, async () => {
    return {
  ...
  app.put('/api/rbac/record-rules/:resource(*)', { preHandler: requirePermFastify('security.permission_set.update') }, async (request, reply) => {
  ...
  app.get('/api/rbac/sessions', { preHandler: requirePermFastify('security.session.list') }, async (request) => {
  ...
  app.delete('/api/rbac/sessions/:id', { preHandler: requirePermFastify('security.session.revoke') }, async (request, reply) => {
    db.prepare(`UPDATE sbos_rbac_sessions SET revoked_at = datetime('now') WHERE id = ?`).run(request.params.id);
    return reply.code(204).send();
  });

  // ───── Audit ─────

  app.get('/api/rbac/audit', { preHandler: requirePermFastify('security.audit.read') }, async (request) => {
