...
function makeMockDb() {
  ...
  function classify(sql) {
    ...
    if (/SELECT.*FROM\s+FINANCE\.CUSTOMERS/.test(s) && /INVOICES/.test(s)) return 'report-join';
    if (/SELECT.*FROM\s+FINANCE\.PAYMENTS/.test(s) && /INVOICES/.test(s)) return 'report-payments-join';
    if (/SELECT.*FROM\s+FINANCE\.INVOICES/.test(s)) return 'report-invoices';
    if (/SELECT.*FROM\s+FINANCE\.PAYMENTS/.test(s)) return 'report-payments';
    return 'passthrough';
  }
  ...
  async function query(sql, params) {
    ...
    if (kind === 'report-payments-join') {
      ...
      for (const inv of invoices.values()) {
        ...
        if (!cust) continue;
        candidateRows.push({ ...inv, name: cust.name, hvhh: cust.hvhh });
      }
    ...
    if (whereMatch) {
      ...
      // Also support status IN (...) pattern: "status IN ($1, $2, ...)" or
      // "status = ANY($1)" (the report uses a fixed list of statuses).
      const inMatch = where.match(/status\s+IN\s*\(([^)]+)\)/i);
      if (inMatch) {
      ...
      // Support aggregations: SUM(amount_amd) → sum across joined payments
      // (handled below in the GROUP BY branch when applicable).
    }

    // ORDER BY
    if (orderBy) {
      const orderClause = orderBy[1].trim();
      // Tolerate "col ASC" / "col DESC" / "col1, col2"
    ...
    if (selectMatch) {
      ...
      // Detect SUM() aggregations
      const sumMatch = cols.match(/SUM\s*\(\s*([^)]+?)\s*\)/i);
      if (sumMatch) {
        // Aggregate the value (assume a single sum column). Caller may
        // SELECT other literals alongside (e.g. "COALESCE(SUM(amount_amd), 0) AS paid_amd").
        ...
        // Pick the alias or build a default one.
        const aliasMatch = cols.match(/AS\s+(\w+)/i);
      ...
      // Detect GROUP BY aggregation: e.g. "SELECT customer_id, SUM(total_amd) AS total_billed_amd ... GROUP BY customer_id"
      if (groupBy) {
        ...
        if (groupColMatch) {
          ...
          for (const [key, rows] of groups) {
            // Pick the first row for non-aggregated columns
            const r = { [groupCol]: key };
            // Pull all the SUM(col) AS alias expressions
            const sumRegex = /SUM\s*\(\s*([^)]+?)\s*\)\s+AS\s+(\w+)/gi;
            let sm;
            while ((sm = sumRegex.exec(cols)) !== null) {
              const sumCol = sm[1];
              const alias = sm[2];
              r[alias] = rows.reduce((acc, row) => acc + Number(row[sumCol] || 0), 0);
            }
            // COUNT(*) AS alias
            const countRegex = /COUNT\s*\(\s*\*\s*\)\s+AS\s+(\w+)/i;
            const cm = cols.match(countRegex);
            if (cm) r[cm[1]] = rows.length;
            // Copy through non-aggregated columns from the first row.
            // We look for `select col1, col2, SUM(x) AS y ...` and project
            // anything that isn't an aggregate.
            const colList = cols.split(',').map((s) => s.trim());
            for (const c of colList) {
              if (/^(SUM|COUNT|COALESCE)\s*\(/i.test(c)) continue;
              if (/^(\w+)\s+AS\s+(\w+)$/i.test(c)) {
                const m = c.match(/^(\w+)\s+AS\s+(\w+)$/i);
                r[m[2]] = rows[0][m[1]];
              } else if (/^\w+$/.test(c)) {
                if (!(c in r)) r[c] = rows[0][c];
              }
            }
            out.push(r);
          }
      ...
      // Wildcard: return all columns of each row.
      if (cols === '*') {
