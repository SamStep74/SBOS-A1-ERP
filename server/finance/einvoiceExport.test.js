...
async function seedCustomer(db, { name, hvhh = null, address = null }) {
  await db.query('INSERT INTO finance.customers (name, hvhh, address) VALUES ($1, $2, $3)', [name, hvhh, address]);
  const r = await db.query('SELECT MAX(id) AS id FROM finance.customers');
  return Number(r.rows[0].id);
}
async function seedInvoice(db, { customer_id, invoice_number, issue_date, due_date, subtotal_amd, vat_amd, total_amd, status = 'sent' }) {
  await db.query(
    ...
    [customer_id, invoice_number, issue_date, due_date, subtotal_amd, vat_amd, total_amd, status, null,
     status !== 'draft' ? '2026-06-01T00:00:00Z' : null, null, null],
  );
...
describe('finance — e-invoice export (real DB)', () => {
  ...
  test('1. exportInvoiceEInvoice: produces a valid XML for a single invoice with 20% VAT lines', async () => {
    ...
    const custId = await seedCustomer(freshDb, { name: 'Acme LLC', hvhh: '99999999', address: '5 Main St' });
    const invId = await seedInvoice(freshDb, {
      customer_id: custId, invoice_number: 'EXP-001', issue_date: '2026-06-15', due_date: '2026-07-15',
      subtotal_amd: 100000, vat_amd: 20000, total_amd: 120000, status: 'sent',
    });
    await seedLine(freshDb, { invoice_id: invId, description: 'Consulting', quantity: 1, unit_price_amd: 100000, line_total_amd: 100000 });
    ...
    assert.ok(r.xml.includes('<TaxId>99999999</TaxId>') || r.xml.includes('<TaxId/>'), 'customer hvhh or empty TaxId');
  ...
  test('4. exportInvoiceEInvoice: throws on missing/invalid supplier', async () => {
    ...
    await assert.rejects(() => exportInvoiceEInvoice(db, 1, { name: '' }), /supplier.name is required/);
  });
  ...
  test('5. exportMonthlyEInvoices: returns one XML per non-void invoice in the month', async () => {
    ...
    const i1 = await seedInvoice(freshDb, { customer_id: custId, invoice_number: 'M-1', issue_date: '2026-06-05', due_date: '2026-07-05', subtotal_amd: 50000, vat_amd: 10000, total_amd: 60000 });
    await seedLine(freshDb, { invoice_id: i1, description: 'A', quantity: 1, unit_price_amd: 50000, line_total_amd: 50000 });
    const i2 = await seedInvoice(freshDb, { customer_id: custId, invoice_number: 'M-2', issue_date: '2026-06-15', due_date: '2026-07-15', subtotal_amd: 80000, vat_amd: 16000, total_amd: 96000 });
    await seedLine(freshDb, { invoice_id: i2, description: 'B', quantity: 1, unit_price_amd: 80000, line_total_amd: 80000 });
    await seedInvoice(freshDb, { customer_id: custId, invoice_number: 'M-3', issue_date: '2026-05-31', due_date: '2026-06-30', subtotal_amd: 10000, vat_amd: 2000, total_amd: 12000 });
    const i4 = await seedInvoice(freshDb, { customer_id: custId, invoice_number: 'M-4', issue_date: '2026-06-20', due_date: '2026-07-20', subtotal_amd: 30000, vat_amd: 6000, total_amd: 36000, status: 'void' });
  ...
  test('6. exportMonthlyEInvoices: empty month returns empty array', async () => {
    ...
    await seedInvoice(freshDb, { customer_id: custId, invoice_number: 'E-1', issue_date: '2026-01-15', due_date: '2026-02-15', subtotal_amd: 10000, vat_amd: 2000, total_amd: 12000 });
    const out = await exportMonthlyEInvoices(freshDb, '2026-06', SUPPLIER);
  ...
  test('8. exportMonthlyEInvoices: 0% VAT invoice (exempt) → VatRate 0 in the XML', async () => {
    ...
    const i = await seedInvoice(freshDb, {
      customer_id: custId, invoice_number: 'EX-1', issue_date: '2026-06-15', due_date: '2026-07-15',
      subtotal_amd: 50000, vat_amd: 0, total_amd: 50000, // exempt: 0 VAT
    });
    await seedLine(freshDb, { invoice_id: i, description: 'Exempt service', quantity: 1, unit_price_amd: 50000, line_total_amd: 50000 });
    const out = await exportMonthlyEInvoices(freshDb, '2026-06', SUPPLIER);
  ...
  test('9. exportInvoiceEInvoice: multiple lines aggregate correctly', async () => {
    ...
    const i = await seedInvoice(freshDb, {
      customer_id: custId, invoice_number: 'MULT-1', issue_date: '2026-06-15', due_date: '2026-07-15',
      subtotal_amd: 150000, vat_amd: 30000, total_amd: 180000,
    });
    await seedLine(freshDb, { invoice_id: i, description: 'A', quantity: 1, unit_price_amd: 50000, line_total_amd: 50000 });
    await seedLine(freshDb, { invoice_id: i, description: 'B', quantity: 1, unit_price_amd: 100000, line_total_amd: 100000 });
  ...
  test('10. e-invoice XML declares the EINVOICE_NAMESPACE URN (wire-format stability)', async () => {
    ...
    const i = await seedInvoice(freshDb, { customer_id: custId, invoice_number: 'URN-1', issue_date: '2026-06-15', due_date: '2026-07-15', subtotal_amd: 10000, vat_amd: 2000, total_amd: 12000 });
    await seedLine(freshDb, { invoice_id: i, description: 'X', quantity: 1, unit_price_amd: 10000, line_total_amd: 10000 });
    const r = await exportInvoiceEInvoice(freshDb, i, SUPPLIER);
  ...
  test('11. inferVatRate returns 0 when subtotal_amd is 0 (defends divide-by-zero)', async () => {
    ...
    // we want to exercise.
    const i = await seedInvoice(freshDb, {
      customer_id: custId, invoice_number: 'ZERO-1', issue_date: '2026-06-15', due_date: '2026-07-15',
      subtotal_amd: 0, vat_amd: 0, total_amd: 0,
    });
    await seedLine(freshDb, { invoice_id: i, description: 'Zero', quantity: 1, unit_price_amd: 0, line_total_amd: 0 });
    const r = await exportInvoiceEInvoice(freshDb, i, SUPPLIER);
  ...
  test('12. monthly export: an invoice with no lines still produces valid XML (empty <Lines/>)', async () => {
    ...
    await seedInvoice(freshDb, {
      customer_id: custId, invoice_number: 'NL-1', issue_date: '2026-06-15', due_date: '2026-07-15',
      subtotal_amd: 0, vat_amd: 0, total_amd: 0,
    });
    const out = await exportMonthlyEInvoices(freshDb, '2026-06', SUPPLIER);
    assert.equal(out.length, 1);
  ...
  test('13. inferVatRate: 0% VAT with positive subtotal still gives 0 (round-trip on the ratio)', async () => {
    ...
    const i = await seedInvoice(freshDb, {
      customer_id: custId, invoice_number: 'EX2-1', issue_date: '2026-06-15', due_date: '2026-07-15',
      subtotal_amd: 100000, vat_amd: 0, total_amd: 100000,
    });
    await seedLine(freshDb, { invoice_id: i, description: 'Exempt', quantity: 1, unit_price_amd: 100000, line_total_amd: 100000 });
    const r = await exportInvoiceEInvoice(freshDb, i, SUPPLIER);
    assert.ok(r.xml.includes('<VatRate>0</VatRate>'));
  ...
  test('14. legacy row with empty invoice_number and missing due_date → empty-string fallbacks', async () => {
    ...
    assert.ok(r.xml.includes('<Number></Number>') || r.xml.includes('<Number/>'),
      'empty invoice_number becomes an empty Number element');
    assert.ok(r.xml.includes('<DueDate></DueDate>') || r.xml.includes('<DueDate/>'),
